...
const checkAnswer = () => {
  ...
  if (currentAnswer === "Lake Superior" || currentAnswer === "Superior") {
    outputBox.innerHTML += `<h3>Lake Superior</h3><p>The worldest largest freshwater lake by suface area.</p>`;
    score += 1;
  } else if (currentAnswer === "Lake Michigan" || currentAnswer === "Michigan") {
    outputBox.innerHTML += `<h3>Lake Michgan</h3><p>The only Great Lake located entirely within the United States.</p>`;
    score += 1;
  } else if (currentAnswer === "Lake Huron" || currentAnswer === "Huron") {
    outputBox.innerHTML +=`<h3>Lake Huron</h3><p>The Great Lake with the longest shoreline.</p>`;
    score += 1;
  } else if (currentAnswer === "Lake Erie" || currentAnswer === "Erie") {
    outputBox.innerHTML += `<h3>Lake Erie</h3><p>The Great Lake with the smallest volume and shallowest.</p>`;
    score += 1;
  } else if (currentAnswer === "Lake Ontario" || currentAnswer === "Ontario") {
    outputBox.innerHTML += `<h3>Lake Ontario</h3><p>The smallest Great Lake, is crucial for water supply and holds histrocial shipwecks.</p>`;
    score += 1;
  } else if (currentAnswer === "Lake Champlain" || currentAnswer === "Champlain") {
    // Secret bonus answer!
  outputBox.innerHTML += `<h3>Lake Champlain</h3><p>You found the sixth Great Lake.</p>`;
  champlain = true;
  } else {
...
const checkScore = () => {
  ...
  if (score === 5 && champlain) {
    statusBox.innerHTML = "A true Great Lake master. All five Great Lakes and the sixth Lake";
